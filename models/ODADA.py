...
class OD_DA(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, bilinear=True):
        ...
        self.avgpool = torch.nn.AvgPool2d(256)

    def forward(self, x):
        f_all = self.feature_extractor(x)
        # print("all_feature.size:{}".format(f_all.size()))

        f_di = self.E_DI(f_all)
        # print("di.feature:{}".format(f_di.size()))
        f_ds = f_all - f_di

        f_di_pool = self.avgpool(f_di).squeeze().squeeze()
        f_ds_pool = self.avgpool(f_ds).squeeze().squeeze()
        # print("f_di_pool:{}".format(f_di_pool.size()))
        # print("f_ds_pool:{}".format(f_ds_pool.size()))
        # print("loss_orthogonal:{}".format(F.cosine_similarity(f_di_pool, f_ds_pool).size()))
        # print("loss_orthogonal1:{}".format(F.cosine_similarity(f_di_pool, f_ds_pool, dim=1)))
        # loss_orthogonal = torch.mean(f_di_pool.square() * f_ds_pool.square(), dim=1)
        # loss_orthogonal = (f_di_pool.square() * f_ds_pool.square()).mean()
        loss_orthogonal = F.cosine_similarity(f_di_pool, f_ds_pool) #f_di_pool:torch.Size([3, 64]),bsq取1就dim0
...
if __name__ == '__main__':
    ...
    for name, param in model.named_parameters():
        ...
        if "outc" in name:
            param.requires_grad = False
        # print(name)
    final, loss_orthogonal, prob_di, prob_ds = model(inp)
    print("final.size:{}".format(final.size()))
    # print("orthogonal.size:{}".format(loss_orthogonal.size()))
    # print("di.size:{}".format(prob_di.size()))
    # print("ds.size:{}".format(prob_ds.size()))
