...
class UNet(nn.Module):
    ...
    def forward(self,x):
        ...
        out9=self.fore3(out8)
        # out10=self.fore1(out9)
        # return out9, out_3, out6, out8
        return out9

def test():
    input = torch.rand(1, 3, 128, 128).cuda()  # 输入 B C H W
    model = UNet().cuda()
    out = model(input)
    print(out.size())
